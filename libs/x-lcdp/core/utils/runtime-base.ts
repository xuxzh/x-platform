...
import { EXTENSION_EVENT_CHANNEL } from '@x/lcdp/model';
import { Subject } from 'rxjs';

export abstract class XRuntimeBase<T> {
  /** 代理池。用于管理动态内容使用的对应数据，当数据发生变更时，通知对应组件 */
  private proxyPools: Map<string, Subject<RhSafeAny>> = new Map();
  /** 数据集代理对象 */
  private datasetProxy: WithNil<T>;
  /** 数据订阅总线 */
  ...
  abstract lodash: RhSafeAny;
  abstract rxjs: RhSafeAny;
  abstract operators: RhSafeAny;
  abstract datefns: RhSafeAny;
  abstract screenfull: RhSafeAny;
  abstract go: RhSafeAny;
  ...
  findComponentSchema(
    ...
    if (list && list.length > 0) {
      let result = null;
      this.lodash.forEach(list, (item: RhSafeAny) => {
  ...
  createLocalSpace(
    ...
    return new RhLocalSpace(target, host, this as RhSafeAny, schema, ev);
  }
  /** 创建操作 */
